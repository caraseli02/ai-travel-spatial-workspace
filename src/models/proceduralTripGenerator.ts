...
export function generateTripFromMessage(message: string): Trip {
  ...
  const matchedKey = Object.keys(destinationMap).find(key => lowerMsg.includes(key));
  let destData = matchedKey ? destinationMap[matchedKey] : null;

  if (!destData) {
    const cityMatch = message.match(/\b(?:to|in)\s+([^,.;!?\d]+?)(?=\s+(?:for|with)\b|\s+\d+\s*(?:day|days|night|nights|person|people|traveler|travelers|guest|guests)\b|[,.;!?]|$)/i);
  ...
  } else if (lowerMsg.includes('for two') || lowerMsg.includes('two guests') || lowerMsg.includes('two people') || lowerMsg.includes('2 people')) {
    travelers = 2;
  }

  let duration = 5;
  const durationMatch = message.match(/(\d+)\s*-?\s*(?:day|days|night|nights)/i);
  if (durationMatch) {
