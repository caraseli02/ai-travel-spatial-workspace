#!/usr/bin/env bash
# Pick the next eligible tech-debt issue (ready-for-agent, unblocked, no open PR).
set -euo pipefail
...
OPEN_PRS_JSON="$(gh pr list --state open --json number,title,body --limit 100)"
...
has_open_pr_for_issue() {
  local num="$1"
  local pattern="#${num}"
  echo "$OPEN_PRS_JSON" | jq -e --arg p "$pattern" '
    any(.[]; (.title + "\n" + .body) | contains($p))
  ' >/dev/null 2>&1
}
...
pick_next_issue() {
  ...
  if [[ "$count" -eq 0 ]]; then
    echo "NO_ELIGIBLE:no open ready-for-agent tech-debt issues" >&2
    return 1
  fi
